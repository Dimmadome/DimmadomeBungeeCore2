...
public class Unban extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        if (BungeePerms.getInstance().getPermissionsChecker().hasPerm(sender.getName(), "soontm.staff")) {
            if (args.length < 2) {
                sender.sendMessage(new TextComponent(ChatColor.RED + "/unban <player> <reason>"));
                return;
            }
            ...
            if (plugin.playerDataContainsPlayer(args[0])) {
                try {
                    ...
                    PreparedStatement checkstatement = plugin.connection.prepareStatement("SELECT length FROM `ban_table` WHERE uuid=?;");
                    ...
                    if (checkset.getString("length") != null) {
                        ...
                        plugin.sendStaffMessage(sender.getName() + " unbanned " + args[0] + ". Reason:" + msg);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
